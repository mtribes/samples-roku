...
function WebSocketClient() as object
    ...
    ws.send = function (message as dynamic, _opcode = -1 as integer, silent = false as boolean, do_close = true as boolean) as integer
        ...
        if type(message) = "roAssociativeArray" then
          message.sig = m._calcHMAC(FormatJSON(message))
          if message.sig = "" then
            return -1
          end if
    ...
    ws._handle_frame = function (opcode as integer, payload as object) as void
        if opcode <> m.OPCODE.PONG
            ...
            frame_print += "  PayloadHEX: " + payload.toHexString()
            frame_print += "  PayloadASCII: " + payload.toAsciiString()
...
'            m.send("", m.OPCODE.PONG)
            ...
            return
        else if opcode = m.OPCODE.CONTINUATION
            m._post_message("on_message", {
                type: 0,
                message: payload.toAsciiString()
            })
    ...
    ws._calcHMAC = function(message) as string
      hmac = CreateObject("roHMAC")
      if hmac.setup("sha256", m.secret) = 0 then
        hash = CreateObject("roByteArray")
        hash.FromAsciiString(message)
        hmac.update(hash)
        hash = hmac.final().ToHexString()
        return LCASE(hash)
      end if
      return ""
    end function
